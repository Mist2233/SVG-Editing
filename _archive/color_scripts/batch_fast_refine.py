...
def optimize_pair(svg_path, png_path, output_path, num_iter=200):
    ...
    # Auto-Scale SVG to match Target Resolution
    ...
    # 4. Optimizer Setup (Exactly as fast_refine.py)
    points_vars = []
    color_vars = []
    ...
    # fast_refine.py settings: Points LR=1.0, Color LR=0.01
    ...
    for t in range(num_iter):
        ...
        # Pure MSE Loss (Exactly as fast_refine.py)
...
def main():
    parser = argparse.ArgumentParser(description="Batch process SVG fast refinement (Pure MSE)")
    ...
    for d in subdirs:
        ...
        svgs = [s for s in svgs if not s.name.endswith("-m.svg") and not s.name.endswith("-v2.svg") and not s.name.endswith("-v3.svg") and not s.name.endswith("-fast.svg")]
        ...
        svg_file = svgs[0]
        png_file = pngs[0]
        output_name = f"{svg_file.stem}-fast.svg"
