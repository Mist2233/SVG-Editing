...
def optimize_pair(svg_path, png_path, output_path, num_iter=200):
    print(f"Processing LineArt: {svg_path} -> {output_path}")
    ...
    # ==========================================
    # 针对 Line Art (线条画) 的“三锁策略”
    # ==========================================

    # 1. 锁形状 & 锁粗细 (Lock Geometry)
    for path in shapes:
        ...
        path.stroke_width.requires_grad = False # 【关键】锁粗细！绝不允许变细！

    # 2. 锁透明度 (Lock Alpha / Force Solid)
    # 强制 Alpha = 1.0，且不加入 optimizer
    rgb_optim_vars = []
    ...
    # 5. Loop
    print(f"--> Start LineArt Refine ({num_iter} iterations)...")
...
def main():
    parser = argparse.ArgumentParser(description="Batch process SVG: Line Art Special (Three Locks)")
    ...
    for d in subdirs:
        ...
        # 排除之前的尝试
        valid_svgs = [s for s in svgs if "external" not in s.name and "fast" not in s.name and "fix" not in s.name and "coloronly" not in s.name and "smart" not in s.name and "solid" not in s.name and "lineart" not in s.name]
        ...
        png_file = pngs[0]
        output_name = f"{svg_file.stem}-lineart.svg"
